...
(function() {
  var Agents, HideMeProxyScraper, Host, Request, nodeio, proxies, qs,
  ...
  */
  ...
  Host = "http://hideme.ru/proxy-list/";
  ...
  */
  ...
  HideMeProxyScraper = (function(_super) {

    __extends(HideMeProxyScraper, _super);

    function HideMeProxyScraper() {
      return HideMeProxyScraper.__super__.constructor.apply(this, arguments);
    }

    HideMeProxyScraper.prototype.input = false;

    HideMeProxyScraper.prototype.run = function() {
      var _this = this;
      return this.getHtml("" + Host + "?" + (qs.stringify(Request)), function(err, $, data) {
        if (_this.err) {
          _this.exit(err);
        }
        console.log(data);
        return $('table.pl tr').not(":first-child").each(function(proxy) {
          var doc;
          data = proxy.find("td");
          doc = {
            host: data.eq(0).text,
          ...
          proxies.push(doc);
          return _this.emit(proxies);
        });
      });
    };

    return HideMeProxyScraper;

  })(nodeio.JobClass);

  this["class"] = HideMeProxyScraper;

  this.job = new HideMeProxyScraper({
    timeout: 20,
    spoof: true
  });
