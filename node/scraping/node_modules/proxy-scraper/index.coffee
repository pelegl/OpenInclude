...
Request = {anon: 4, type: "s", maxtime: 1000}
...
class HideMeProxyScraper extends nodeio.JobClass
  input: false
  run: ->
    @getHtml "#{Host}?#{qs.stringify(Request)}", (err, $, data)=>
      @exit err if @err
      
      console.log data
        
      $('table.pl tr').not(":first-child").each (proxy)=>
        data = proxy.find "td"
        doc = 
          host: data.eq(0).text
          port: data.eq(1).text
...
@job   = new HideMeProxyScraper {timeout: 20, spoof: true}
