...
module.exports =
  ...
  comment: (req, res) ->
    Task.findByIdAndUpdate(req.params.id, {$push:
        {comments:
          {text: req.body.comment, author: req.user.github_display_name, author_link: req.user.github_username, date: new Date()}}},
    (result, data) ->
  ...
  start: (req, res) ->
    Worklog.findOne({"who.id": req.user._id, task: req.params.id}, (result, log) ->
      ...
      Task.findByIdAndUpdate(req.params.id, {person:
        {id: req.user._id, name: req.user.github_display_name, link: req.user.github_username}}).exec()
    ...
    Worklog.findOne({"who.id": req.user._id, task: req.params.id}, (result, log) ->
      ...
      Task.findByIdAndUpdate(req.params.id, {person: null, $inc:
        {logged: diff} }).exec()
